...
MINARDO.heatmap = { // Heat Map Highlighter
	...
	"init": function (data, svg, x, y, width, height){
		...
		this.rowOrder = Object.keys(minardo_data);
	...
	"initRows": function (){	// used to be called buildHeatmap
		...
		this.rFullWidth = this.rWidth * minardo_data[Object.keys(minardo_data)[0]].time_course.length; // this is silly. Fix it.
...
//					console.log(row_data);
	...
	"sorting": {
		"order": Object.keys(minardo_data),
...
MINARDO.data = {
	...
	"init": function(){
		...
		this.numberOfTimePoints = minardo_data[Object.keys(minardo_data)[0]].time_course.length;
		...
		// process cluster data. Not really necissary, only needed to do it once. Now it is stored in minardo_data3.js
/*		var clusterLookup = {};

		// process the "clusters" global array, from "minardo_data3.js"
		cluster.data.forEach(function (spectra){
			var this_cluster = null;
			var cluster_value = 0;
			var id = spectra.id;
			
			MINARDO.data.allClusters.forEach(function (cluster){
				if (spectra[cluster] > cluster_value) {
					cluster_value = spectra[cluster];
					this_cluster = cluster;
				}
			});
			clusterLookup[id] = this_cluster;
		});
...
//		console.log("write a function here to get the cluster based on ID...");
	...
	"numberOfRows": Object.keys(minardo_data).length,
