...
MINARDO.racetrack = { // Race Track Highlighter
	"init": function (){
		drawMajorTracks();
		drawPoints();
		drawOtherPoints();
	...
	"unselect": function (){ //does this even do anything?
		...
		MINARDO.racetrack.unhighlight(); //not needed?
	},
	"highlightAllRelated": function (spectra){
		MINARDO.helper.highlight.brushProtein(spectra);
//		MINARDO.hmh.brushRelated(spectra);
		if (MINARDO.racetrack.lookup[spectra]){
			MINARDO.racetrack.lookup[spectra].forEach(function (thing){
				MINARDO.racetrack.highlight(thing);
			});
		}
	},
...
//		console.log(id);
		...
		if (MINARDO.racetrack.lookup[id]){
			MINARDO.racetrack.lookup[id].forEach(function (spectra){ //highlight each instance of this target
				MINARDO.racetrack.highlight(spectra);
...
/* don't do this.. why would you want it to do this? not sure why, but it only activates for IRS1??
				if (spectra.datum().relatedSpectra) {
					spectra.datum().relatedSpectra.forEach(function (relatedSpectraID){
						if (MINARDO.racetrack.lookup[relatedSpectraID]){
							MINARDO.racetrack.lookup[relatedSpectraID].forEach(function (relatedSpectra){
								MINARDO.racetrack.highlight(relatedSpectra);
							});
						}
					});
				} else {
//					console.log("no related spectra found");
	...
	"highlight": function (thing){
		thing.datum().hitbox.attr("fill", MINARDO.racetrack.hitboxColourActivator);
		MINARDO.racetrack.highlighted.push(thing);
//		console.log(thing.datum().relatedSpectra);
	},
	"unhighlight": function (){
		MINARDO.helper.highlight.unselect();
		MINARDO.racetrack.highlighted.forEach(function (thing){
			if (thing.classed("point")) {
				thing.datum().hitbox.attr("fill", MINARDO.racetrack.hitboxColourDefault);
			} else if (thing.classed("track")) {
				thing.datum().hitbox.attr("stroke", MINARDO.racetrack.hitboxColourDefault);
			}
		});
		MINARDO.racetrack.highlighted = [];
	},
	"highlighted": [],
...
//		console.log("Highlighting track : "+ track.datum().name);
		...
		track.datum().ownSpectra.forEach(function (spectra){
			MINARDO.racetrack.highlightSpectra(spectra);
		});

		track.datum().targetSpectra.forEach(function (spectra){
			MINARDO.racetrack.highlightTargetSpectra(spectra);
		});

		MINARDO.racetrack.highlighted.push(track);
		track.datum().hitbox.attr("stroke", MINARDO.racetrack.hitboxColourActivator);
	},
	"highlightSpectra": function (spectra){

//		MINARDO.hmh.highlightActivator(spectra, MINARDO.racetrack.hitboxColourActivator);
		MINARDO.helper.highlight.brushProtein(spectra);
	...
	"highlightTargetSpectra": function (spectra){
		if (MINARDO.racetrack.showingTargets) {

			//MINARDO.hmh.highlightTarget(spectra, MINARDO.racetrack.hitboxColourTarget);
			MINARDO.helper.highlight.brushProtein(spectra);

			if (MINARDO.racetrack.lookup[spectra]) {
				MINARDO.racetrack.lookup[spectra].forEach(function (thing){
					thing.datum().hitbox.attr("fill", MINARDO.racetrack.hitboxColourTarget);
					MINARDO.racetrack.highlighted.push(thing);
				});
			}
		}
	...
	"showTargets": function (){
		if(MINARDO.racetrack.showingTargets) {
			MINARDO.racetrack.showingTargets = false;
			d3.select("#show_targets_button").attr("fill", "#aaaaaa");
		} else {
			MINARDO.racetrack.showingTargets = true;
			d3.select("#show_targets_button").attr("fill", MINARDO.racetrack.hitboxColourTarget);
		}
	...
	var tracks = d3.select("#major_tracks").selectAll(".track")
		...
		.each(function (d, i){
			...
			var hitbox = track.append("svg:path")
				.attr("opacity", 0.75)
				.attr("fill", "none")
				.attr("stroke", MINARDO.racetrack.hitboxColourDefault)
				.attr("stroke-width", 12)
				.on("mouseenter",function (d,i){
					MINARDO.racetrack.highlightTrack(track);
				})
				.on("mouseout",function (d,i){
					MINARDO.racetrack.unhighlight();
				});
			if (d.path) {
...
},
{
	...
	"targetSpectra": [11851,25972,25965,8439,8448,6505,6246,21925,33202,19781,19782,13271,13269,33657,33654,13766,4944]
},
{
	"name": "mTORC2",
	...
	"targetSpectra": [3081,11843,11851,25965,6505,8448,18051]
},
{
	"name": "AS160",
...
},
{
	...
	"targetSpectra": [11441,11430,33125,32018,32814,10125,10126,7476,7477,23370,1800,26145,24474,4288,17539,8254,7591,12914,4508,32613,25879,26145] //also ULK1 S775
},
{
	"name": "Erk1/2",
	...
	"targetSpectra": [1212,2165,6242,2170,11092,11593,12995,13015]
},
{
	"name": "Gsk",
...
},
{
	...
	"targetSpectra": [1143,1144,8828,32553,33654]
},
{
	"name": "PKA",
...
function drawPoints(){
	d3.select("#Gene_Names").selectAll(".point")
		...
		.each(function(d, i) {
			...
			MINARDO.racetrack.points[d.number] = this;
			MINARDO.racetrack.lookupPush(this);
		});
}
...
//	console.log("hey");
	d3.select("#Gene_Names").selectAll(".otherPoint")
		...
		.each(function(d, i) {
			...
			MINARDO.racetrack.otherPoints[d.number] = this;
			MINARDO.racetrack.lookupPush(this);
		});
}
...
function drawHitbox(thing, data){
	...
	var hitbox = d3.select(thing).append("svg:rect")
		...
		.attr("y", coords.y)
		.attr("x", coords.x)
		.attr("fill", MINARDO.racetrack.hitboxColourDefault) //this is just so we can see it for now, change the alpha to zero later.
		.on("mouseenter",function (d,i){
			var target = d.targetSpectra || d.target;
			MINARDO.racetrack.highlightAllRelated(target);
		})
		.on("mouseout",function (d,i){
			MINARDO.racetrack.unhighlight();
		});
...
//<text transform="matrix()" fill="#F15A29" font-family="'MyriadPro-Bold'" font-size="11.2" class="FS-11.2 tag tag-S186"></text>
  ...
  "target": 13015, //also: 12995, 12994
  "proteinSpectra": [37449,33117,37331,31143]
}];
