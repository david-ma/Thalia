...
//						var pos = MINARDO.racetrack.getRandomPos(d, Math.floor(i/2));
						...
						.each("end", function(d,i){
							//draw labels
							MINARDO.racetrack.drawLabel(id, pos);
	...
	"hoverBox": {
		...
		"init": function() {
			...
			d3.selectAll(".node")
				...
				.on("click", MINARDO.racetrack.hoverBox.click);
			...
			that.drawButton(racetrack, "edit",  20, 20, 100, 50, "button 1", MINARDO.racetrack.hoverBox.toggleEditText);
			that.drawButton(racetrack, "move", 130, 20, 100, 50, "button 1", MINARDO.racetrack.hoverBox.toggleMoveBoxes);
			that.drawButton(racetrack, "auto", 240, 20, 100, 50, "button 1", callback);
			that.drawButton(racetrack, "load", 350, 20, 100, 50, "button 1", callback);
			that.drawButton(racetrack, "save", 460, 20, 100, 50, "button 1", callback);
		...
		"drawButton": function(thing, id, x, y, w, h, text, callback){
			console.log('woadsfjnewuifeiwfhuiehuaio');
			console.log(thing);
			var group = thing.insert("svg:g", "#box_0")
		...
		"editText": function(d){
			...
			if(editedText != null){
				MINARDO.data.comments[MINARDO.racetrack.hoverBox.currentTarget.id] = editedText;
		...
		"mouseover": function(){
			if(MINARDO.racetrack.hoverBox.showcomments) {
				var id = $(this).attr("id").substr(5),
						x = parseInt($("#node_"+id+" circle").attr("cx")),
						y = parseInt($("#node_"+id+" circle").attr("cy")),
