...
//					console.log(MINARDO.data.comments[d.id]);
	...
	"buildLink": function(connection, blob, type){
				if (!MINARDO.racetrack.linkExists(blob.a_id, connection.a_id)
				&& blob.a_id != connection.a_id
				&& typeof MINARDO.racetrack.drawnNodes[connection.a_id] != 'undefined'){

					var link = {
						"node": blob.a_id,
						"target": connection.a_id
					};
					var s = {
						'x': blob.pos.x,
						'y': blob.pos.y
					...
					f = {
						'x': connection.pos.x - s.x,
						'y': connection.pos.y - s.y
					};
					...
					if(typeof blob.links == 'undefined') {
						...
						blob.links.going.push(link);
					}
					
					if(typeof connection.links == 'undefined') {
						connection.links = {'coming':[link],
													'going':[]
													}
					} else {
						connection.links.coming.push(link);
					}
...
// add a button for comments
		...
		"draw_tracks": function(){
			var tracks = d3.select("#racetrack")
				.insert("svg:g", "#links").attr("id","tracks_group");
			// find out which things should be tracks.
			var kinases = MINARDO.data.getMostActiveKinases();
			// remove their nodes.
			// remove their links.
			for(var i = 0; i < 5; i++) {
				console.log(kinases[i]);
				var p = Object.keys(kinase_data[kinases[i]].phosphosites);
				console.log(p);
...
//						console.log();
						MINARDO.racetrack.designate_track(kinases[i], blob);
						
						
						
						// turn a blob into a track.
						// find out if it belongs to a track already..
						// find the first instance, find the last instance...
						// bind it to that track.
					}
				});
			}
			...
			// draw a track which goes all the way around for this node.
			// select the top 5 most cluttered paths?
			// bind them to tracks.
			
			// draw a track.

				/*
			MINARDO.racetrack.draw_track(tracks, 50, 50, 200);
			MINARDO.racetrack.draw_track(tracks, 100, 50, 200);
			MINARDO.racetrack.draw_track(tracks, 150, 50, 200);
			MINARDO.racetrack.draw_track(tracks, 200, 50, 200);
			MINARDO.racetrack.draw_track(tracks, 250, 50, 200);
			MINARDO.racetrack.draw_track(tracks, 300, 50, 200);
			MINARDO.racetrack.draw_track(tracks, 350, 50, 200);
			MINARDO.racetrack.draw_track(tracks, 380, 50, 200);
/*			MINARDO.racetrack.draw_track(tracks, 80, 50, 200);
			MINARDO.racetrack.draw_track(tracks, 140, 50, 200);
			MINARDO.racetrack.draw_track(tracks, 200, 500, 200);
			MINARDO.racetrack.draw_track(tracks, 260, 50, 550);
			MINARDO.racetrack.draw_track(tracks, 320, 50, 350);
			MINARDO.racetrack.draw_track(tracks, 380, 50, 250);*/
		},
		"listOfTracks": {},
		"designate_track": function(track, blob){
			var that = this;
			var listOfTracks = that.listOfTracks;
			if (typeof listOfTracks[track] == 'undefined'){
				listOfTracks[track] = [blob];
			} else {
				listOfTracks[track].push(blob);
			}
			//get the earliest and latest... and draw the track to these specifications.
			that.getLimitsOf(listOfTracks[track]);
		},
		"places": {},
		"getLimitsOf": function(tracks){
//			console.log(tracks);
			tracks.forEach(function(track){
				track.links.coming.forEach(function(link){
					var id = MINARDO.racetrack.blobs[link.target].d_id;
					if (typeof MINARDO.racetrack.places[id] == 'undefined') {
						var place = MINARDO.data.getPlaceOf(id),
	...
	//					console.log(dat);
						if(typeof dat[1] != 'undefined'){
							posX = 0;
							console.log("bleh this one sucks");
						} else if (typeof dat[0] != 'undefined'){
						console.log(id);
							console.log(dec);
							posX = dat[0].x + (dat[0].w * dec);
						} else if (typeof dat[2] != 'undefined'){
							posX = dat[2].x + (dat[2].w * (1 - dec));
						}

	console.log("pushing "+ posX);
						MINARDO.racetrack.places[id] = posX;

	//					var a = MINARDO.data.getFirstActivation();
	//					console.log(link);
	//					console.log(a);
					}
				});
		...
		"draw_track": function(tracks, r, start, end){
			var a = 500 - r,
					b = 2 * r,
					c = r + 500;
...
//				console.log("dragging");
		...
		"updateNode": function(blob, m, transition){
			...
			if (blob.links) {
				blob.links.going.forEach(function(link){
					var target = MINARDO.racetrack.blobs[link.target];
					link.s = {
							'x': m[0],
							'y': m[1]
							}
					...
					if (transition) {
						link.line.transition().attr("d", "M "+link.s.x+","+link.s.y+" l "+link.f.x+","+link.f.y+" z");
					} else {
						link.line.attr("d", "M "+link.s.x+","+link.s.y+" l "+link.f.x+","+link.f.y+" z");
					}
				});

				blob.links.coming.forEach(function(link){
					var node = MINARDO.racetrack.blobs[link.node];
					var target = MINARDO.racetrack.blobs[link.target];
